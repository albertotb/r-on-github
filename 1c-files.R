...
year <- 2015
repos <- sort(unique(readRDS(paste0("repos", year, ".rds"))))

repo_info <- function(repo) {
  cache_path <- paste0("cache-repo-fun/", gsub("/", "-", repo), ".rds")
  ...
  files <- github_get(paste0(base, "/repos/", repo, "/git/trees/master?recursive=1"))
  filelist <- unlist(lapply(files$tree, "[", "path"), use.names = F)
  r_files <- grep("\\.[Rr]$", filelist, value = TRUE)
  ...
  out <- list(info = info, languages = lng, dir = paths, desc = desc, tags = tag_names, r_files = r_files)
